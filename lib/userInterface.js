...
//==LINK CONFIG==\\
let link0 = window.localStorage.getItem('link0') || 'https://www.youtube.com/';
let link0Name = psl.get(extractHostname(link0));


//==SET LINK TEXT==\\
const setLinkText = (selectedCity, cityLinkName) => {
  const cityEnglish = document.getElementById('label' + selectedCity + '-english');
  cityEnglish.innerHTML = cityLinkName;
}
setLinkText('0', link0Name);


//==INTERFACE==\\
//make listeners for hover over event
export const userInterface = () => {
  const Label0 = document.getElementById('label0-bg');
