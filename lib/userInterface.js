...
const setLink = (selectedCityId) => {
  ...
  //cut a websites domain short if it is too long.
  const cityWidth = cityCoordinateArray[selectedCityId].labelDimensions[0]
  if(cityWidth < 83){
    linkNames[selectedCityId] = linkNames[selectedCityId].slice(0,7) + '...';
  }else if(cityWidth < 166 && linkNames[selectedCityId].length > 14){
    linkNames[selectedCityId] = linkNames[selectedCityId].slice(0,14) + '...';
  }
...
export const userInterface = () => {
  ...
  const makeContextmenuListener = (cityId) => {
    const listener = (e) => {
      e.preventDefault();
      let result = window.prompt('Please enter valid link.');
      if(result){
