...
export const userInterface = () => {
  ...
  const makeContextmenuListener = (selectionName) => {
    const listener = (e) => {
      e.preventDefault();
      //TODO change location of the switch to inside the result if statement
      switch (selectionName){
        case 'aoyamaitchome' :
          ...
          if(result){
            ...
            aoyamaitchomeLink = result;
            aoyamaitchomeLinkName = psl.get(extractHostname(aoyamaitchomeLink));
          }
        break;
      }
      return;
